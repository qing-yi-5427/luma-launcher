...
调研时间：2026-08-31。目标不是复制功能，而是提炼对“稳定、美观、低占用”有直接帮助的做法。
...
- 对 Luma 的采用：保留查询代次与取消令牌；结果先呈现，再异步补图标；图标缓存设硬上限并在隐藏后收缩。
...
- 对 Luma 的采用：维持应用与 Everything 两个轻量搜索源，不引入常驻插件宿主，不生成缩略图，只读取小尺寸 Shell 图标。
...
1. 键入路径只做必要工作：预归一化应用索引、65 ms 防抖、旧查询取消和代次校验。
2. 首屏结果不等待图标；图标异步、冻结后回写，并使用有上限的 LRU 缓存。
